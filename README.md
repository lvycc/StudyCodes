...
## 1.15 删除排序链表中的重复元素
* 我的做法
    * 定义两个node节点对象，now节点表示当前最后一个不重复的节点，next节点表示遍历节点
    * 通过next节点循环遍历链表，并比较now和next的值是否相同，如果相同则记录标记需要修改next指针
    * 循环结束返回head即可
* 官网做法
    * 官网通过递归实现，当递归当前节点为null或者递归当前节点的next节点为null则返回该节点递归结束
    * 通过递归获取下一节点，获取到下一节点后判断当前节点和下一节点的值是否相同则将下一节点作为当前节点
    * 递归结束返回head节点
